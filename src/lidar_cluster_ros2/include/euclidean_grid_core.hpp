...
private:
  ...
  void lidar_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr input_msg)
  {
    ...
    // Convert to PCL data type (PointXYZ — Patchwork++ output has no intensity)
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr voxel_map_ptr(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::VoxelGrid<pcl::PointXYZ> voxel_grid;
    voxel_map_ptr = createVoxelGrid(cloud, voxel_grid, voxel_leaf_size, min_points_number_per_voxel);
